...
public interface SkupinaRepository extends CrudRepository<Skupina, Long> {
    ...
    @Query(value = "SELECT s.naziv, u.uporabniskoIme, u.stSkupin FROM Skupina s INNER JOIN skupina_uporabnik su ON s.id=su.skupina_id INNER JOIN Uporabnik u ON su.uporabnik_id=u.id WHERE s.stUporabnikov < 4", nativeQuery = true)
